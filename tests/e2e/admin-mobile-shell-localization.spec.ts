...
const OUTPUT_DIR = path.join('outputs', 'issue-323-admin-mobile-shell-subtitle')
...
async function attachScreenshot(page: Page, testInfo: TestInfo) {
  ...
  const screenshotPath = path.join(OUTPUT_DIR, `admin-mobile-shell-${testInfo.project.name}.png`)
  await page.screenshot({ path: screenshotPath, fullPage: true })
  await testInfo.attach('admin mobile shell screenshot', {
    path: screenshotPath,
    contentType: 'image/png',
  })
}

test('Spanish admin mobile shell uses localized title and subtitle', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 390, height: 844 })
  await loginAsAdmin(page)
  await page.waitForLoadState('networkidle', { timeout: 10_000 }).catch(() => undefined)

  await expect(page.getByText('Administración').first()).toBeVisible()
  await expect(page.getByText('Gestión de la plataforma').first()).toBeVisible()
  await expect(page.getByText('Platform management')).toHaveCount(0)
